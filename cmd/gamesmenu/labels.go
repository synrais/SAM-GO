...
// Options -> Display & Sorting -> Menu list options and saved to the [Menu] section of SAM.ini.
...
// applyMenuConfig loads the saved menu settings from SAM.ini's [Menu].
...
// saveMenuConfig stores the current menu settings in SAM.ini's [Menu].
...
// cycles to its next value. Changes are saved to SAM.ini on the way out.
