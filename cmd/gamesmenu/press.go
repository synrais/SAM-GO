...
// -------------------------
// Virtual Press Test (gamesmenu.sh -press ...)
// -------------------------

// testPress presses buttons on SAM's virtual pad or keyboard, to check
// they reach a running core, e.g. "gamesmenu.sh -press start" with a game
// on its title screen.
func testPress(args []string) {
	seq := strings.Join(args, " ")
	steps, err := mister.ParseSequence(seq)
	if err != nil || len(steps) == 0 {
		fmt.Println("Usage: gamesmenu.sh -press <buttons...>")
	...
	fmt.Println("Created SAM's virtual device, waiting for MiSTer to pick it up...")
	nodes := samDeviceNodes()
	if len(nodes) == 0 {
		fmt.Println("  !! No input device called SAM found in /proc/bus/input/devices")
	}
	for _, n := range nodes {
	...
	if opened {
		fmt.Println("  MiSTer has opened SAM's device: yes")
	} else {
		fmt.Println("  !! MiSTer has NOT opened SAM's device, so it can't see the presses")
	}
...
// samDeviceNodes finds the /dev/input/event* nodes of devices named "SAM".
func samDeviceNodes() []devNode {
	...
	for _, block := range strings.Split(string(b), "\n\n") {
		if !strings.Contains(block, `Name="SAM"`) {
			continue
		}
		info := ""
