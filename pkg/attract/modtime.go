...
// Returns (modified?, currentTimestamp, error)
func isFolderModified(systemId, folderPath string, savedTimestamps FolderTimestamps) (bool, time.Time, error) {
	...
	if !exists || !savedTimestamp.Equal(currentTimestamp) {
		return true, currentTimestamp, nil
	}

	return false, currentTimestamp, nil
}
...
func checkAndHandleModifiedFolder(systemId, folderPath, gamelistDir string, savedTimestamps FolderTimestamps) (bool, error) {
	...
	modified, currentTimestamp, err := isFolderModified(systemId, folderPath, savedTimestamps)
	if err != nil {
		fmt.Printf("Error: %s\n", err)
		return false, err
	}
	...
	if modified {
		...
		if err := saveTimestamps(gamelistDir, savedTimestamps); err != nil {
			return false, err
		}
