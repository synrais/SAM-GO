...
// in SAM.ini, with [StaticDetector.X] overrides for a system or group.
...
//
//	ssh -t root@mister "watch -n 0.1 cat /tmp/SAM_detector"
const StatusFile = config.TempFolder + "/SAM_detector"
...
type Detector struct {
	...
	// viewOnly: just measure and show (gamesmenu -watch outside attract
...
// its status, for "gamesmenu -watch" when attract mode isn't running:
...
// Latest returns the most recent snapshot, for debug output.
func (d *Detector) Latest() StaticEvent {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.latest
}
...
func (d *Detector) writeStatus(cfg config.StaticDetectorConfig, force bool) {
	...
	fmt.Fprintf(&b, "SAM static detector - updated %s\n\n", time.Now().Format("15:04:05.0"))
	if title == "" {
	...
	for {
		...
		if uptime > currCfg.Grace {
			if avgHex == "#000000" && staticScreenRun > currCfg.BlackThreshold && !handledBlack {
				...
				case currCfg.WriteBlackList && lists.Add(lists.Blacklist, systemID, title) == nil:
			...
			if avgHex != "#000000" && staticScreenRun > currCfg.StaticThreshold && !handledStatic {
				...
				case currCfg.WriteStaticList && lists.AddStatic(systemID, title, staticStartTime) == nil:
