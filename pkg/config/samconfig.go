...
// SAMFolder is where SAM and its SAM.ini live.
const SAMFolder = ScriptsFolder + "/.MiSTer_SAM"

// SAMIniFile is the one config file shared by SAM, the games menu and
// everything else. Set SAM_CONFIG to use a different file.
const SAMIniFile = SAMFolder + "/SAM.ini"
...
type AttractConfig struct {
	...
	IdleRestart int `ini:"idlerestart"`
}
...
type StartupConfig struct {
	Start string `ini:"start"` // Nothing, Games menu, Attract mode
...
type AutoInputConfig struct {
	...
	Menu      bool              `ini:"menu"`    // after games menu launches
...
// MenuConfig holds the games menu's list label settings. Values are stored
...
type Config struct {
	Path     string
	Created  bool // SAM.ini didn't exist and was written from the default
...
// IniPath returns the SAM.ini in use: SAM_CONFIG if set, otherwise SAMIniFile.
func IniPath() string {
...
// Load reads SAM.ini, writing the embedded default first if it's missing.
...
func Load() (*Config, error) {
	cfg := &Config{
		...
		Startup:                 StartupConfig{Start: "Nothing"},
		Menu:                    MenuConfig{ShowManufacturer: true, ArcadeFirst: true},
		StaticDetector: StaticDetectorConfig{
	...
	if _, err := os.Stat(cfg.Path); os.IsNotExist(err) {
		...
		if err := os.WriteFile(cfg.Path, assets.DefaultSAMIni, 0644); err != nil {
			return cfg, fmt.Errorf("write default SAM.ini: %w", err)
		}
		cfg.Created = true
	}
	...
	// are read as Database rules so existing SAM.ini files keep working.
	for _, sec := range file.Sections() {
...
// moveOldDatabase moves the games database from its old location
// (Scripts/.config/sam/menu.db) to MenuDb, once, so nobody has to rebuild.
func moveOldDatabase() {
	if _, err := os.Stat(MenuDb); err == nil {
		return
	}
	if _, err := os.Stat(oldMenuDb); err != nil {
		return
	}
	_ = os.MkdirAll(filepath.Dir(MenuDb), 0755)
	if err := os.Rename(oldMenuDb, MenuDb); err != nil {
		// Different drives: copy, then remove the old one.
		if data, err := os.ReadFile(oldMenuDb); err == nil {
			if os.WriteFile(MenuDb, data, 0644) == nil {
				_ = os.Remove(oldMenuDb)
			}
		}
	}
}
...
// SaveMenu writes cfg.Menu into the [Menu] section of SAM.ini. Only those
...
// Options lines left in an older SAM.ini are ignored.
