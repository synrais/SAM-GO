...
// "system_folder = ID:/path" lines in SAM.ini. Each is that system's own
...
func GetSystemPaths(cfg *config.Config, systems []System) []PathResult {
	...
	for _, system := range systems {
		...
		// Extra folders just for this system (system_folder in SAM.ini).
		for _, folder := range SystemFolders(cfg, system) {
			if info, err := os.Stat(folder); err != nil || !info.IsDir() || seen[folder] {
				continue
			}
			seen[folder] = true
			matches = append(matches, PathResult{system, folder})
		}
	}
...
func GetAllSystemPaths(cfg *config.Config) []PathResult {
...
func GetPopulatedGamesFolders(cfg *config.Config, systems []System) map[string][]string {
