...
// Game list ordering
//
// How folders and games are ordered in the games menu and search results
// ([Menu] FolderPosition, GameOrder and IgnoreThe in SAM.ini).
...
// SortResults orders search results by game name, then system, then
// extension.
func SortResults(results []SearchResult, o GameOrder, systemName func(id string) string) {
	sort.SliceStable(results, func(i, j int) bool {
		a, b := results[i], results[j]
		if a.Name != b.Name {
			return o.Less(a.Name, b.Name)
		}
		if sa, sb := systemName(a.SystemId), systemName(b.SystemId); sa != sb {
