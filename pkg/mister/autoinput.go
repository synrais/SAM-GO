...
// BIOS skip
//
// Presses buttons on SAM's virtual gamepad (or keys on its virtual
...
const autoInputPid = "/tmp/gamesmenu_autoinput.pid"
...
// as its own background process ("gamesmenu -autoinput <sequence>"), so it
// keeps going after the games menu exits, and any earlier one still
