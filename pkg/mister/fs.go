...
func MatchRbf(rbfs []string, match string) string {
	...
	for _, rbf := range rbfs {
		...
		// Match "name_<date>.rbf" or a clean "name.rbf", but not a longer core
		// name that merely starts the same (e.g. NeoGeoPocket vs NeoGeoPocket-Color).
		lowerFile, lowerMatch := strings.ToLower(file), strings.ToLower(match)
		if strings.HasPrefix(lowerFile, lowerMatch+"_") || lowerFile == lowerMatch+".rbf" {
			if len(parts) == 1 {
				return match
			} else {
...
func GetMounts(cfg *config.Config) ([]string, error) {
