...
// (the games menu leaves the console itself).
// OpenMenuLog records each step of opening the games menu on the TV.
const OpenMenuLog = "/tmp/gamesmenu_openmenu.log"
...
func RunOnScreen(exe string, args ...string) error {
	...
	defer kbd.Close()
	stepLog("created SAM's virtual keyboard")
	...
	stepLog("switched to console 2 (%s), starting the games menu there", activeTty())
	...
	stepLog("games menu on console 2 ended: err=%v %s", err, strings.TrimSpace(string(out)))
	return err
}

// OpenGamesMenu closes whatever game is running (loading the MiSTer menu
// core) and opens the games menu on the TV, straight into its Search
...
func OpenGamesMenu(search bool) error {
	...
	stepLog("opening the games menu on the TV (search=%v)", search)
	if !IsMenuRunning() {
